...
pub fn this_is_our_mutex_lock_before_handle_function<T: ?Sized>(mutex: &Mutex<T>) {
    println!("Mutex locking {:?}, this is before", addr_of!(*mutex));
}
//#[rustc_std_internal_symbol]
pub fn this_is_our_mutex_lock_after_handle_function<T: ?Sized>(mutex: &Mutex<T>, ret: &mut LockResult<MutexGuard<'_, T>>) {
    println!("Mutex locking {:?}, this is after, ret addr {:?}", addr_of!(*mutex), addr_of!(*ret));
}
